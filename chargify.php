...
class ChargifyComponent extends Object {
  ...
  function createSubscription() {
    ...
    $result = $this->sendRequest('subscriptions',null,'POST',$data);
    return $result;
  }
  ...
  function changeProduct($id,$data) {
    $result = $this->sendRequest('subscriptions/'.$id ,null,'PUT',$data);
    return $result;
  }
  ...
  function updateSubscription($id,$subscription) { 
    $result = $this->sendRequest('subscriptions/'.$id ,null,'PUT',$subscription);
    return $result;
  }
  
  
  /**
   * Build new/updated subscription
   * 
   * @param array $subscription array of the new subscription details
   */
    ...
    $jsonSubscription = '{"subscription":{
      "product_handle":"'.$this->data['Subscription']['handle'].'",
      "customer_attributes":{
      ...
      "credit_card_attributes":{
        "full_number":"'.$this->data['Subscription']['full_number'].'",
        "expiration_month":"'.$this->data['Subscription']['expiration_month'].'",
        "expiration_year":"'.$this->data['Subscription']['expiration_year'].'",
        "billing_address":"'.$this->data['Subscription']['address_line_one'].'",
        "billing_city":"'.$this->data['Subscription']['city'].'",
        "billing_state":"'.$this->data['Subscription']['county'].'",
        "billing_zip":"'.$this->data['Subscription']['post_code'].'",
        "billing_country":"'.$this->data['Subscription']['country_id'].'"
      }
    }}';
	...
	*/
	...
	/**
   *  Builds the JSOIN to update the product associated with 1 subscription
   */
   
  /* 
	function buildProductUpdateJson() {
    $jsonSubscription = '{"subscription":{"product_handle":"'.$this->data['Subscription']['handle'].'"}}'; 
    return $jsonSubscription;
	}
  */
